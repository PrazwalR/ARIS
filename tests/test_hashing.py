...
def test_same_account_yields_same_risk_id_at_every_bank():
    assert risk_id_for_account("ACC-999", KEY) == risk_id_for_account("ACC-999", KEY)
    assert len(risk_id_for_account("ACC-999", KEY)) == 64


def test_different_accounts_differ():
    assert risk_id_for_account("ACC-999", KEY) != risk_id_for_account("ACC-998", KEY)


def test_different_keys_yield_different_ids():
    other = bytes.fromhex("f0e1d2c3" * 8)
    assert risk_id_for_account("ACC-999", KEY) != risk_id_for_account("ACC-999", other)
...
def test_is_hmac_not_plain_suffix_hash():
    ...
    assert risk_id_for_account("ACC-999", KEY) != legacy
...
def test_surrounding_whitespace_is_formatting_noise():
    ...
    assert risk_id_for_account("ACC-999\n", KEY) == risk_id_for_account("ACC-999", KEY)


def test_separators_stay_significant():
    """Folding '-' away would merge accounts a bank may treat as distinct."""
    assert risk_id_for_account("ACC-999", KEY) != risk_id_for_account("ACC999", KEY)


def test_non_string_account_rejected():
    with pytest.raises(TypeError):
        risk_id_for_account(12345, KEY)  # type: ignore[arg-type]


def test_short_key_rejected():
    with pytest.raises(ValueError):
        risk_id_for_account("ACC-999", b"short")
...
@pytest.mark.parametrize(
    "crafted",
    ["\u01312345678", "\u00df2345678", "\ufb002345678", "\u017f2345678"],
)
def test_unicode_case_folding_cannot_forge_a_collision(crafted: str):
    ...
    with pytest.raises(ValueError):
        risk_id_for_account(crafted, KEY)
